label carter1:
    ...
    with Fade(1.5, 1, 2)
    nvlbazar "Неоня сидел за столом и писал фанфик про дяду Диму Скита."
    nvlbazar "Получалось нечто очень смешное."
    nvlbazar "А начинался он так:"
    nvl clear
    ...
    scene kohnyashvilli
    with ed_night_dis
    nvlbazar "Неоня отложил ручку."
    nvlbazar "Вместо нормального фанфика про дядю Диму Скита шла какая-{w=0.2}то ёбань."
    nvlbazar "Его мысли смог развеять лишь упавший с потолка Вадим Морозов."
    nvl clear
    ...
    show volga
    with flash
    vsn "Хорошо вчера побухали..."
    vchn "Тут базару 0."
    vchn "Но всё же..."
    play sound team_govoril
    extend " Сколько я тебе раз говорил?" with vpunch
    vchn "Не стоит сука малыша на абсенте мешать."
    window hide
    play sound team_suka
    with vpunch
    pause 0.3
    play sound kup_mat
    with vpunch
    pause 0.5
    vchn "После всего этого - булки с ленты меня не особо спасают..."
    vchn "От похмела ебучего..."
    vsn "Шоп булки тебя не спасали?"
    window hide
    play sound team_dane
    pause 1.5
    vchn "В этой ёбаной жизни возможно всё."
    vchn "Даже такое."
    "Вишневский заметил что-то вдали..."
    vsn "Это чё за хуня?"
    vchn "СВДшка ещё в багажнике?"
    vsn "Я её оттуда не убирал."
    vchn "Вот и отлично."
    stop music fadeout 5
    ...
    window hide
    $ renpy.notify("Делай звук громче")
    pause 3
    ...
    $ renpy.movie_cutscene('source/videosos/dobermanchik.webm')
    hide deadlylxrd
    show deadlylxrd:
        subpixel True default
        parallel:
    ...
    pause 22
    $ renpy.movie_cutscene('source/videosos/skitsret1.webm')
    play sound tolkan4
    window hide
    pause 3
    play sound dostal
    ...
    scene skittolkan2
    pause 0.8
    $ renpy.movie_cutscene('source/videosos/skitsret2.webm')
    scene skitupalvtolkan
    play sound tolkan6
    window hide
    pause 12
    "Вечный не стал медлить."
    ...
    play ambience bar_amb fadein 5
    scene bar
    show deadlylxrd at right
    ...
    $ renpy.movie_cutscene('source/videosos/buhlo.webm')
    nn "И когда это было?"
    brg "29 числа."
    nn "Это же вчера..."
    brg "Ага."
    ...
    "И начался пиздец."
    scene black
    $ renpy.movie_cutscene('source/videosos/basketbolnogoy.webm')
    play sound fb
    play music otec
    scene ext_path_day
    show bergennorm
    ...
    stop sound
    $ renpy.movie_cutscene('source/videosos/lxrddrift.webm')
